...
class IbtHtmlReporter implements Reporter {
  ...
  private render(): string {
    ...
    const total = functionalEntries.length || 1;
    const successful = passed + warnings;
    ...
    const healthLabel = failed > 0 ? 'Needs attention' : warnings > 0 ? 'Review warnings' : 'All systems go';
    const healthClass = failed > 0 ? 'critical' : warnings > 0 ? 'caution' : 'healthy';
...
.test-card{border:1px solid #dfe7ee;border-left:0;border-radius:16px;margin-bottom:16px;box-shadow:0 7px 24px #1b344b0b;transition:box-shadow .25s ease,transform .25s ease,outline-color .25s ease}.test-card:hover{box-shadow:0 12px 32px #1b344b16}.test-card:before{content:"";position:absolute}.test-card>summary{min-height:82px;padding:18px 22px;border-left:6px solid var(--red);background:linear-gradient(100deg,#fff,#f7fafc)}.test-card.setup-card>summary{border-left-color:#8090a0}.test-card.passed-warning>summary{border-left-color:#e6a700;background:linear-gradient(100deg,#fffdf7,#fff9df)}.test-card.passed-warning .test-number{background:linear-gradient(135deg,#e6a700,#bd8500);box-shadow:0 6px 14px #9a680030}.test-card[open]>summary{border-bottom:1px solid #e5ebf0}.test-card>summary:after{content:"⌄";font-size:22px;transition:transform .2s ease}.test-card[open]>summary:after{content:"⌄";transform:rotate(180deg)}.test-card.is-active{position:relative;z-index:3;outline:3px solid #2684ff;outline-offset:3px;box-shadow:0 18px 44px #1266a838,0 0 0 8px #2684ff12;transform:translateY(-2px)}.test-card.is-active>summary{background:linear-gradient(100deg,#edf6ff,#fff 58%,#eef5ff)}.test-card.is-active .test-name:after{content:"OPEN";display:inline-flex;margin-left:10px;padding:3px 7px;border-radius:999px;background:#dcebff;color:#095fae;font-size:9px;font-weight:900;letter-spacing:.8px;vertical-align:2px}.test-card.is-active .test-number{box-shadow:0 7px 18px #1266a84d}.test-number{border-radius:10px;background:linear-gradient(135deg,var(--red),#bd1230);box-shadow:0 6px 14px #d7193530}.test-name{font-size:17px}.badge{letter-spacing:.35px}.body{padding:24px 26px 28px}.meta{display:inline-flex;padding:7px 11px;border-radius:8px;background:#f2f6f9;color:#607184;font-size:13px}
...
.reading-progress{position:fixed;z-index:100;inset:0 auto auto 0;width:0;height:4px;background:linear-gradient(90deg,#2684ff,#8d55ff,#ff4964);box-shadow:0 0 16px #2684ff90;transition:width .08s linear}.floating-tools{position:fixed;z-index:60;right:22px;bottom:22px;display:grid;gap:9px}.float-button{width:46px;height:46px;border:1px solid #ffffff70;border-radius:15px;background:#142d44e8;color:#fff;font-size:18px;cursor:pointer;box-shadow:0 12px 28px #10223535;backdrop-filter:blur(12px);transition:transform .2s ease,background .2s ease}.float-button:hover{transform:translateY(-3px) rotate(3deg);background:#e31937}.back-top{opacity:0;pointer-events:none;transform:translateY(8px)}.back-top.visible{opacity:1;pointer-events:auto;transform:none}
@property --pass{syntax:"<number>";inherits:false;initial-value:0}@property --warning{syntax:"<number>";inherits:false;initial-value:0}.health-overview{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:22px;padding:22px 48px;border-bottom:1px solid var(--line);background:linear-gradient(110deg,#edf5fc,#fff 52%,#fff5f7)}.health-donut{--pass:0;--warning:0;position:relative;width:112px;height:112px;border-radius:50%;background:conic-gradient(var(--green) 0 calc(var(--pass)*1%),#e6a700 calc(var(--pass)*1%) calc((var(--pass) + var(--warning))*1%),#d71935 calc((var(--pass) + var(--warning))*1%) 100%);box-shadow:0 12px 28px #17334c20;transition:--pass 1s ease,--warning 1s ease}.health-donut:before{content:"";position:absolute;inset:13px;border-radius:50%;background:var(--surface);box-shadow:inset 0 0 0 1px var(--line)}.health-donut-center{position:absolute;inset:0;display:grid;place-content:center;text-align:center}.health-donut-center strong{font-size:25px;line-height:1}.health-donut-center span{margin-top:4px;color:var(--muted);font-size:9px;font-weight:800;letter-spacing:.8px;text-transform:uppercase}.health-copy .health-kicker{margin:0 0 5px;color:var(--muted);font-size:10px;font-weight:850;letter-spacing:1.4px;text-transform:uppercase}.health-copy h2{margin:0;font-size:26px;letter-spacing:-.6px}.health-copy p{margin:7px 0 0;color:var(--muted);font-size:13px}.health-state{display:flex;align-items:center;gap:9px;padding:11px 15px;border-radius:999px;font-size:12px;font-weight:850}.health-state:before{content:"";width:9px;height:9px;border-radius:50%;animation:status-pulse 1.8s ease-in-out infinite}.health-state.healthy{background:#dbf7e8;color:#087a46}.health-state.healthy:before{background:#13a05f}.health-state.caution{background:#fff0b3;color:#765500}.health-state.caution:before{background:#e6a700}.health-state.critical{background:#ffe1e5;color:#9b2733}.health-state.critical:before{background:#d71935}
.test-card>summary{position:relative}.test-card>summary:before{content:"";position:absolute;left:0;bottom:0;width:var(--duration-width,0);height:3px;background:linear-gradient(90deg,#2684ff,#9d6cff);opacity:.7;transition:width .9s cubic-bezier(.2,.8,.2,1)}.issue-drawer{position:fixed;z-index:90;top:0;right:0;width:min(420px,92vw);height:100vh;padding:26px;background:#fffffff5;box-shadow:-24px 0 60px #10223530;backdrop-filter:blur(20px);transform:translateX(105%);transition:transform .35s cubic-bezier(.2,.8,.2,1);overflow:auto}.issue-drawer.open{transform:none}.drawer-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:20px}.drawer-head h2{margin:0;font-size:24px}.drawer-head p{margin:5px 0 0;color:var(--muted);font-size:13px}.drawer-close{width:36px;height:36px;border:1px solid var(--line);border-radius:11px;background:var(--surface);color:var(--ink);font-size:20px;cursor:pointer}.issue-item{width:100%;display:block;margin:10px 0;padding:15px;border:1px solid var(--line);border-left:5px solid #e6a700;border-radius:12px;background:var(--surface);color:var(--ink);text-align:left;cursor:pointer;box-shadow:0 6px 18px #17334c0c;transition:transform .18s ease,box-shadow .18s ease}.issue-item.failed,.issue-item.timedOut,.issue-item.interrupted{border-left-color:#d71935}.issue-item:hover{transform:translateX(-4px);box-shadow:0 10px 24px #17334c18}.issue-item strong{display:block;margin-bottom:5px}.issue-item span{color:var(--muted);font-size:11px;font-weight:700;text-transform:uppercase}.drawer-empty{padding:22px;border:1px dashed var(--line);border-radius:12px;color:var(--muted);text-align:center}.drawer-backdrop{position:fixed;z-index:80;inset:0;background:#07182766;opacity:0;pointer-events:none;transition:opacity .3s ease}.drawer-backdrop.open{opacity:1;pointer-events:auto}::highlight(search-hit){background:#ffe36c;color:#172538}.celebration{position:fixed;z-index:120;inset:0;pointer-events:none;overflow:hidden}.confetti{position:absolute;top:-20px;width:9px;height:16px;border-radius:3px;animation:confetti-fall 3.8s cubic-bezier(.18,.75,.35,1) forwards}@keyframes confetti-fall{to{transform:translate3d(var(--drift),105vh,0) rotate(780deg);opacity:.25}}
body.dark{--ink:#e7eff7;--muted:#9eb0c1;--line:#30475c;--surface:#12283c;--soft:#172f45;background:radial-gradient(circle at 8% 0%,#163b61 0,transparent 30%),radial-gradient(circle at 94% 12%,#4b1828 0,transparent 27%),#071522;color:var(--ink)}body.dark .wrap{background:#0c2032f5;border-color:#ffffff16}body.dark .summary,body.dark .health-overview{background:linear-gradient(115deg,#10283d,#0b1d2e 55%,#261622)}body.dark .metric,body.dark .test-card,body.dark .test-card>.body,body.dark .dispatch-job,body.dark .dispatch-body,body.dark .report-action,body.dark .global-search,body.dark .dashboard-toolbar,body.dark .equipment-filter{background:#12283c;color:var(--ink);border-color:var(--line)}body.dark .test-card>summary{background:linear-gradient(100deg,#142c42,#0e2235)}body.dark .test-card.passed-warning>summary{background:linear-gradient(100deg,#2c2a19,#1b2833)}body.dark .test-card.is-active{outline-color:#55a8ff;box-shadow:0 18px 48px #0009,0 0 0 8px #55a8ff1c}body.dark .test-card.is-active>summary{background:linear-gradient(100deg,#1c4668,#142c42 58%,#1d3953)}body.dark .test-card.is-active .test-name:after{background:#1d5f91;color:#dff1ff}body.dark .meta,body.dark .dispatch-summary div,body.dark .equipment-summary div,body.dark .equipment-summary button,body.dark td,body.dark .job{background:#162f45;color:var(--ink);border-color:var(--line)}body.dark th{background:#061625}body.dark tbody tr:nth-child(even) td{background:#10283c}body.dark tbody tr:hover td{background:#234965!important;color:#f4f9ff!important}body.dark tbody tr.equipment-warning:hover td{background:#66551f!important;color:#fff7d2!important}body.dark tbody tr.equipment-problem:hover td,body.dark tbody tr.validation-row-failed:hover td{background:#652c39!important;color:#ffe9ed!important}body.dark .duplicate-card{background:#241c2a;border-color:#793144}body.dark .duplicate-card h4{color:#ffabb8}body.dark .duplicate-card h4 span{color:#c5aab1}body.dark .duplicate-card.critical-phone{border-color:#ff4964;background:linear-gradient(135deg,#3e1421,#241b29);box-shadow:0 14px 34px #0008,0 0 0 5px #ff496414}body.dark .duplicate-card.critical-phone:before{background:#ff4964;color:#240610;box-shadow:0 0 18px #ff49645c}body.dark .duplicate-card.critical-phone h4,body.dark .critical-phone-note{color:#ffc5ce}body.dark .critical-phone-note{background:#ff496417}body.dark .phone-critical{color:#ff91a2}body.dark .duplicate-card .table-wrap{background:#12283c}body.dark .issue-drawer{background:#0c2032f8}body.dark .report-action:hover,body.dark .report-action.active{background:#1d3b55}body.dark .global-search::placeholder{color:#8fa3b5}
@media(max-width:700px){.health-overview{grid-template-columns:auto 1fr;padding:20px 18px}.health-state{grid-column:1/-1;width:max-content}.health-donut{width:88px;height:88px}.health-donut:before{inset:11px}.health-copy h2{font-size:20px}.floating-tools{right:12px;bottom:12px}.issue-drawer{padding:20px}}
@media(prefers-reduced-motion:reduce){.health-donut,.issue-drawer,.drawer-backdrop,.confetti,.test-card>summary:before{transition:none!important;animation:none!important}}
@media print{body{background:#fff}.wrap{margin:0;box-shadow:none;border:0}.report-hero{min-height:auto;padding:28px 34px;background:#102a40!important}.hero-aurora,.hero-rings,.hero-pills,.hero-scroll-line,.floating-tools,.reading-progress,.issue-drawer,.drawer-backdrop,.celebration{display:none!important}.dashboard-toolbar,.report-actions{display:none!important}.test-card{break-inside:auto}.test-card[hidden]{display:none}.content{padding:22px}.summary{padding:18px 22px}.metric{box-shadow:none}}
</style></head><body><div class="reading-progress" aria-hidden="true"></div><main class="wrap"><header class="report-hero"><span class="hero-aurora one"></span><span class="hero-aurora two"></span><span class="hero-rings"></span><div class="hero-content"><div class="hero-copy"><div class="hero-brand"><span class="hero-mark">IBT</span><span class="hero-kicker"><span class="hero-live-dot"></span>Quality Automation</span></div><h1><span class="hero-gradient">IBT Hub Test Report</span></h1><div class="hero-pills"><span class="hero-pill">Automated validation</span><span class="hero-pill">Live evidence</span><span class="hero-pill">Audit-ready report</span></div></div><div class="header-meta"><div class="header-meta-item report-date"><span>Report Date</span><strong>${this.escape(reportDate)}</strong></div><div class="header-meta-item"><span>Generated</span><strong>${this.escape(generatedAt)}</strong></div><div class="header-meta-item${setupEntry?.status === 'passed' ? ' azure-passed' : ''}"><span>Azure setup</span><strong>${setupEntry?.status.toUpperCase() ?? 'NOT RUN'}</strong></div></div></div><span class="hero-scroll-line"></span></header><section class="health-overview"><div class="health-donut" data-pass="${passedPercent}" data-warning="${warningPercent}"><div class="health-donut-center"><strong data-count="${successPercent}">0</strong><span>% passed</span></div></div><div class="health-copy"><p class="health-kicker">Run health</p><h2>${healthLabel}</h2><p>${failed > 0 ? `${failed} failing test${failed === 1 ? '' : 's'} require review.` : warnings > 0 ? `${warnings} test${warnings === 1 ? '' : 's'} passed with warnings.` : 'Every functional validation completed successfully.'}</p></div><div class="health-state ${healthClass}">${healthLabel}</div></section><section class="summary"><button class="metric tests active" type="button" data-status-filter="all"><strong data-count="${functionalEntries.length}">0</strong><span>Functional Tests</span></button><button class="metric success" type="button" data-status-filter="passed"><strong data-count="${passed}">0</strong><span>Clean Passed</span></button><button class="metric warning-metric" type="button" data-status-filter="warning"><strong data-count="${warnings}">0</strong><span>Warnings</span></button><button class="metric failure" type="button" data-status-filter="failed"><strong data-count="${failed}">0</strong><span>Failed</span></button></section><section class="content"><div class="section-heading"><div><h2>Test Cases</h2><span>Search, filter, and open a test to explore its evidence</span></div><div class="report-actions"><button class="report-action" type="button" data-action="expand">Expand all</button><button class="report-action" type="button" data-action="collapse">Collapse all</button></div></div><div class="dashboard-toolbar"><label class="search-wrap"><input class="global-search" type="search" placeholder="Search Job, equipment, Foreman, Inspector..." autocomplete="off" aria-label="Search report"></label><div class="toolbar-actions"><button class="report-action" type="button" data-action="issues">Issues panel</button><button class="report-action" type="button" data-action="print">Print / PDF</button><span class="visible-count"></span></div></div><div class="no-results">No tests match the selected filter or search.</div>${testCards}</section></main><aside class="issue-drawer" aria-label="Report issues" aria-hidden="true"><div class="drawer-head"><div><h2>Issues &amp; warnings</h2><p>Jump directly to findings that need attention.</p></div><button class="drawer-close" type="button" aria-label="Close issues panel">×</button></div><div class="issue-list"></div></aside><div class="drawer-backdrop"></div><div class="floating-tools"><button class="float-button theme-toggle" type="button" aria-label="Toggle dark mode" title="Dark mode">☾</button><button class="float-button back-top" type="button" aria-label="Back to top" title="Back to top">↑</button></div><div class="celebration" aria-hidden="true"></div><script>
...
var reducedMotion=window.matchMedia('(prefers-reduced-motion: reduce)').matches;document.querySelectorAll('[data-count]').forEach(function(counter){var target=Number(counter.dataset.count);if(reducedMotion){counter.textContent=String(target);return;}var start=performance.now();function tick(now){var progress=Math.min((now-start)/800,1);counter.textContent=String(Math.round(target*(1-Math.pow(1-progress,3))));if(progress<1)requestAnimationFrame(tick);}requestAnimationFrame(tick);});var donut=document.querySelector('.health-donut');if(donut){requestAnimationFrame(function(){donut.style.setProperty('--pass',donut.dataset.pass);donut.style.setProperty('--warning',donut.dataset.warning);});}
var maxDuration=Math.max.apply(null,cards.map(function(card){return Number(card.dataset.duration)||0;}));cards.forEach(function(card){card.style.setProperty('--duration-width',(maxDuration?Math.max(4,(Number(card.dataset.duration)||0)/maxDuration*100):0)+'%');});
var progressBar=document.querySelector('.reading-progress');var backTop=document.querySelector('.back-top');function updateScroll(){var max=document.documentElement.scrollHeight-innerHeight;var percent=max>0?scrollY/max*100:0;progressBar.style.width=percent+'%';backTop.classList.toggle('visible',scrollY>500);}addEventListener('scroll',updateScroll,{passive:true});updateScroll();backTop.addEventListener('click',function(){scrollTo({top:0,behavior:'smooth'});});
var themeButton=document.querySelector('.theme-toggle');var savedTheme='';try{savedTheme=localStorage.getItem('ibt-report-theme')||'';}catch(error){void error;}if(savedTheme==='dark'||(!savedTheme&&matchMedia('(prefers-color-scheme: dark)').matches))document.body.classList.add('dark');function syncTheme(){var dark=document.body.classList.contains('dark');themeButton.textContent=dark?'☀':'☾';themeButton.title=dark?'Light mode':'Dark mode';}syncTheme();themeButton.addEventListener('click',function(){document.body.classList.toggle('dark');syncTheme();try{localStorage.setItem('ibt-report-theme',document.body.classList.contains('dark')?'dark':'light');}catch(error){void error;}});
var drawer=document.querySelector('.issue-drawer');var backdrop=document.querySelector('.drawer-backdrop');var issueList=document.querySelector('.issue-list');var issueCards=cards.filter(function(card){return !card.classList.contains('setup-card')&&card.dataset.status!=='passed';});if(issueCards.length){issueCards.forEach(function(card){var button=document.createElement('button');var name=document.createElement('strong');var status=document.createElement('span');button.type='button';button.className='issue-item '+card.dataset.status;name.textContent=card.querySelector('.test-name').textContent;status.textContent=card.querySelector('.badge').textContent;button.append(name,status);button.addEventListener('click',function(){closeDrawer();card.hidden=false;card.open=true;card.scrollIntoView({behavior:'smooth',block:'start'});});issueList.appendChild(button);});}else{var emptyIssue=document.createElement('div');emptyIssue.className='drawer-empty';emptyIssue.textContent='No issues or warnings in this run.';issueList.appendChild(emptyIssue);}function openDrawer(){drawer.classList.add('open');backdrop.classList.add('open');drawer.setAttribute('aria-hidden','false');}function closeDrawer(){drawer.classList.remove('open');backdrop.classList.remove('open');drawer.setAttribute('aria-hidden','true');}document.querySelector('.drawer-close').addEventListener('click',closeDrawer);backdrop.addEventListener('click',closeDrawer);addEventListener('keydown',function(event){if(event.key==='Escape')closeDrawer();});
function updateSearchHighlights(query){if(!globalThis.CSS||!CSS.highlights)return;CSS.highlights.delete('search-hit');if(query.length<2)return;var ranges=[];cards.filter(function(card){return !card.hidden;}).forEach(function(card){var walker=document.createTreeWalker(card,NodeFilter.SHOW_TEXT);var node;while((node=walker.nextNode())){if(node.parentElement&&['SCRIPT','STYLE'].includes(node.parentElement.tagName))continue;var text=node.textContent.toLowerCase();var from=0;while((from=text.indexOf(query,from))!==-1){var range=new Range();range.setStart(node,from);range.setEnd(node,from+query.length);ranges.push(range);from+=query.length;}}});if(ranges.length)CSS.highlights.set('search-hit',new Highlight(...ranges));}
...
search.addEventListener('input',applyView);function setActiveCard(activeCard){cards.forEach(function(card){var active=card===activeCard&&card.open;card.classList.toggle('is-active',active);if(active)card.setAttribute('aria-current','true');else card.removeAttribute('aria-current');});}cards.forEach(function(card){card.addEventListener('toggle',function(){if(card.open)setActiveCard(card);else if(card.classList.contains('is-active'))setActiveCard(null);saveView();});});
...
document.querySelectorAll('.report-action').forEach(function(button){button.addEventListener('click',function(){var action=button.dataset.action;if(action==='expand'||action==='collapse'){cards.filter(function(card){return !card.hidden;}).forEach(function(card){card.open=action==='expand';});}if(action==='issues')openDrawer();if(action==='print')window.print();saveView();});});
try{var saved=JSON.parse(localStorage.getItem(storageKey)||'null');if(saved){currentFilter=saved.filter||'all';search.value=saved.search||'';var openSet=new Set(saved.open||[]);cards.forEach(function(card){card.open=openSet.has(card.dataset.cardId);});}else{cards.forEach(function(card){card.open=['passed-warning','failed','timedOut','interrupted'].includes(card.dataset.status);});}}catch(error){void error;}applyView();if(${failed}===0&&${warnings}===0&&!reducedMotion){var celebration=document.querySelector('.celebration');var colors=['#2684ff','#13a05f','#ff4964','#e6a700','#9d6cff'];for(var i=0;i<45;i+=1){var piece=document.createElement('span');piece.className='confetti';piece.style.left=Math.random()*100+'vw';piece.style.background=colors[i%colors.length];piece.style.setProperty('--drift',(Math.random()*220-110)+'px');piece.style.animationDelay=Math.random()*.8+'s';celebration.appendChild(piece);}setTimeout(function(){celebration.remove();},5000);}
</script></body></html>`;
  }
  ...
  private renderEntry(entry: ReportEntry, testNumber: number | undefined): string {
    ...
    const error =
      ...
      !entry.inspectorValidation
        ...
        : '';
    return `<details class="test-card ${statusClass} ${isSetup ? 'setup-card' : ''}" data-card-id="${isSetup ? 'setup' : `test-${testNumber}`}" data-status="${statusClass}" data-duration="${entry.duration}"><summary><span class="test-head-left"><span class="test-number">${isSetup ? 'SETUP' : `TEST ${testNumber}`}</span><span><span class="test-name">${this.escape(displayName)}</span><span class="test-original">${this.escape(entry.title)}</span></span></span><span class="test-status"><span class="status-dot ${statusClass}"></span><span class="badge ${statusClass}">${statusLabel}</span></span></summary><div class="body"><div class="meta">Duration: ${(entry.duration / 1000).toFixed(1)}s${entry.jobNumbers ? ` &bull; Extracted: ${entry.jobNumbers.count} Job Numbers` : ''}${entry.dispatchDetails ? ` &bull; Jobs inspected: ${entry.dispatchDetails.jobCount}` : ''}</div>${error}${evidence}</div></details>`;
  }
